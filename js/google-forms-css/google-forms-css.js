...
var googleFormsCSS = function(params) {
  ...
  // request
  jQuery.get('https://amoinvitar.com/js/google-forms-css/google-forms-css-cors.php?url=' + formURL, function(data) {
    ...
    items.forEach(function(item, index) {
      ...
      if (type === 'section') {
        ...
        // title
        if (title) {
          var titleEl = jQuery('<div class="h3"></div>');
          titleEl.text(title);
      ...
      if (title || description) {
        ...
        var labelEl = jQuery('<p class="h4 mt-4"></p>');
        ...
        // description
        if (description) {
          var descriptionEl = jQuery('<p class="text-black-50 text-small mb-1"></p>');
      ...
      if (type === 'checkbox' || type === 'radio') {
        ...
        options.forEach(function(item, index) {
          ...
          // other
          if (item === '') {
            var otherInput = jQuery('<input class="form-control border border-dark" type="text">');
      ...
      // select
        ...
        var select = jQuery('<select class="form-control border border-dark"></select>');
      ...
      // email / text / textarea
        ...
        if (type === 'textarea') {
          var input = jQuery('<textarea class="form-control border border-dark" rows="3"></textarea>');
        } else {
          var input = jQuery('<input class="form-control border border-dark">');
          input.attr('type', type);
        }
    ...
    jQuery('#google-forms-css-form').append('<div class="form-group"><button class="btn button-gris text-uppercase" type="submit">ENVIAR</button></group>');
